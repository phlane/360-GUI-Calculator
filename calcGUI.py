...
class CalculatorGUI:
    def __init__(self, window):
        ...
        tk.Label(self.history_frame, text="(Double-click to revert)", font=('Arial', 10, 'italic'), fg="gray").pack()

        self.history_listbox = tk.Listbox(self.history_frame, width=40, height=10)
        self.history_listbox.pack(padx=5, pady=5)

        tk.Button(self.history_frame, text='Clear History', command=self.clear_history).pack(pady=5)

        self.history_listbox.bind('<Double-1>', self.revert_history_item)
        ...
        tk.Label(self.var_frame, text="(Double-click to insert)", font=('Arial', 10, 'italic'), fg="gray").pack()


        self.var_listbox = tk.Listbox(self.var_frame, width=40, height=10)
        self.var_listbox.pack(padx=5, pady=5)
        self.var_listbox.bind('<Double-1>', self.revert_history_item)

        tk.Button(self.var_frame, text='Clear Variables', command=self.clear_variables).pack(pady=5)

        self.history = []
        self.variables = {}

        self.history_listbox.bind('<Double-1>', self.revert_history_item)
        self.var_listbox.bind('<Double-1>', self.revert_var_item)

        window.bind('<Return>', self.enter_press)
        window.bind('<KP_Enter>', self.enter_press)
            ...
            if result is None:
                ...
                if '=' in self.entry.get():
                    ...
                    if len(self.history) > 10:
                        ...
                        self.window.after(2000, self.enable_entry)

    def enable_entry(self):
        self.entry.config(state='normal')
        self.clear()

    def revert_history_item(self, event):
        try:
    ...
    def revert_var_item(self, event):
        try:
